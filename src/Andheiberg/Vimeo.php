...
class Vimeo
{
	...
	private function _request($method, $call_params = array(), $request_method = 'GET', $url = self::API_REST_URL, $cache = true, $use_auth_header = true)
	{
		...
		if ($this->_cache_enabled and ($cache and $response = $this->_getCached($all_params)))
		{
			return $response;
		}
		...
		// Cache the response
		if ($this->_cache_enabled and $cache)
		{
	...
	public function enableCache($type, $path, $expire = 600)
	{
		...
		if ($this->_cache_enabled == self::CACHE_FILE)
		{
			...
			foreach ($files as $file)
			{
				...
				if (substr($file, -6) == '.cache' and ($last_modified + $expire) < time())
				{
	...
	public static function url_encode_rfc3986($input)
	{
		if (is_array($input))
		{
			return array_map(array('Andheiberg\Vimeo', 'url_encode_rfc3986'), $input);
		}
